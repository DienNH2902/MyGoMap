 ...
 *   "Vị trí hiện tại - Nơi đến" + nút đổi lại điểm đến. Bản đồ, số liệu tuyến
 *   đường (RouteStatsBar), và nút "Chỉ đường" (NavigationControls) vẫn hiển
 *   thị như cũ vì chúng đọc thẳng từ `planner`, không đổi gì cả.
 */
export function QuickDestinationCard({
      ...
      <div className="pointer-events-auto absolute left-1/2 top-4 z-40 w-[92%] max-w-xl -translate-x-1/2 rounded-2xl border border-ink/10 bg-ink/90 p-3 shadow-xl backdrop-blur-md">
        ...
        {/* <div className="mt-3 flex items-center justify-between gap-2">
          <Button
            ...
            type="button"
            onClick={quickSearch.resetSearch}
            className="border border-white/10 bg-black/20 text-cream/80 hover:bg-white/10"
          >
            Đổi điểm đến
          </Button>
          {planner.isLoading && <LoadingSpinner label="Đang tính đường..." />}
        </div> */}
      </div>
    );
  }
  ...
  return (
    <div className="pointer-events-auto absolute left-1/2 top-4 z-40 w-[92%] max-w-md -translate-x-1/2 rounded-2xl border border-ink/10 bg-ink/80 shadow-xl backdrop-blur-md">
