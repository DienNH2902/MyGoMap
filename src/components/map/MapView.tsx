...
export function MapView({ start, end, route, stops, activeStopId, onSelectStop }: MapViewProps) {
    ...
    const applyRoute = () => {
      ...
      if (existingSource && 'setData' in existingSource) {
        ...
        map.addLayer({
          ...
          paint: { 'line-color': '#FF6A1A', 'line-width': 5, 'line-opacity': 0.9 },
        });
      }
  ...
  useEffect(() => {
    ...
    endpoints.forEach(({ place, kind }) => {
      ...
      el.className =
        kind === 'start'
          ? 'flex h-8 w-8 items-center justify-center rounded-full border-2 border-white bg-emerald-500 text-sm font-bold text-white shadow-lg'
          : 'flex h-8 w-8 items-center justify-center rounded-full border-2 border-white bg-ink text-sm font-bold text-white shadow-lg';
  ...
  useEffect(() => {
    ...
    stops.forEach((stop) => {
      ...
      const marker = new Marker({ element: el }).setLngLat([stop.lon, stop.lat]).addTo(map);
